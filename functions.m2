...
testSw = (S,w) -> (
    tscS := TSC(S);
    inSw := sub(inw(w,tscS), ring tscS);
    limitSw := sub(limitTSC(ring tscS, matroidSubdivision(S,w)), ring tscS);
    return inSw == limitSw 
    )    

testS = (S,i,d,n) -> (
    tscS := TSC(S);
    trop := tropicalize(tscS, (value("syms"|d|n))#i, (value("signs"|d|n))#i); 
    return all(trop#"relativeInteriorVectors", w->testSw(S,w))
    )

testSWithVector = (S,v,i,d,n) -> (
    tscS := TSC(S);
    trop := tropicalizeWithVector(tscS, syms37#i, signs37#i); 
    return all(trop#"relativeInteriorVectors", w->testSw(S,w))
    )
...
tropicalizeNoSym = GrS -> (
    inFileName := makeTropicalStartingConeFile(ring GrS, GrS_*); 
    outFileName := temporaryFileName () | ".txt"; 
    tropFileName :=  temporaryFileName () | ".txt"; 
    run ("gfan_tropicalstartingcone < "|inFileName|" > "|outFileName);
    makeTropicalTraverseFileNoSym(outFileName);
    run ("gfan_tropicaltraverse < "|outFileName|" > "|tropFileName);
    return tropFileName
    )
...
noLeaves = G -> not any(vertices(G), v->isLeaf(G,v))
